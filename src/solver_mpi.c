...
int shifted_lopbicg_mpi(CSR_Matrix *A_loc_diag, CSR_Matrix *A_loc_offd, INFO_Matrix *A_info, double *x_loc_set, double *r_loc, double *sigma, int sigma_len, int seed) {
...
#pragma omp parallel private(j)
{
    ...
    if (myid == 0 && k % OUT_ITER == 0) {
        printf("%d, %d, %e, %e, %e\n", k, sigma_len - stop_count, seed_iter_time, matvec_iter_time, shift_iter_time);
    }
#endif
